...
import { sitesApi, emissionFactorsApi } from '@/services/api';
...
export default function EditSiteScreen() {
  ...
  const { updateSite, isLoading } = useSitesStore();
  ...
  const {
    ...
    defaultValues: { name: '', location: '', constructionYear: '', totalAreaM2: '', parkingSpaces: '', annualEnergyKwh: '', employeeCount: '', energyFactorCode: 'ELECTRICITY_FR', materials: [] },
  });
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const [{ data: site }, { data: factors }] = await Promise.all([
          sitesApi.getById(Number(id)),
          emissionFactorsApi.list(),
        ]);
        if (cancelled) return;
        setMaterialFactors(factors.filter((f: Factor) => f.category === 'MATERIAL'));
        setEnergyFactors(factors.filter((f: Factor) => f.category === 'ENERGY'));
        reset({
          name: site.name,
          location: site.location ?? '',
          constructionYear: site.constructionYear ? String(site.constructionYear) : '',
          totalAreaM2: String(site.totalAreaM2),
          parkingSpaces: String(site.parkingSpaces),
          annualEnergyKwh: String(site.annualEnergyKwh),
          employeeCount: String(site.employeeCount),
          energyFactorCode: site.energyFactorCode ?? 'ELECTRICITY_FR',
          materials: site.materials.map((m: { factorCode: string; quantityKg: number }) => ({
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Card style={{ marginBottom: 24 }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            ...
            <TouchableOpacity
              ...
              style={{ backgroundColor: Colors.primaryLight, borderRadius: 6, paddingHorizontal: 12, paddingVertical: 6 }}
            >
          ...
          {fields.length === 0 && (
            <Text style={{ color: Colors.textSecondary, fontSize: 13, textAlign: 'center', paddingVertical: 8 }}>
              Aucun matériau — ajoutez-en pour affiner le calcul de construction.
            </Text>
          )}
        ...
        <Button
          title="Recalculer l'empreinte carbone"
